...
"""
Create a subroute /publication_affiliations
"""
router = APIRouter(
    prefix="/publication_affiliations",
    tags=["publication_affiliations"],
    responses={404: {"description": "Not found"}},
)


@router.get("", response_model=list[schemas.PublicationAffiliation])
def get_publication_affiliations(
    pagination: PaginationParams = Depends(get_pagination_params),
    db: Session = Depends(get_db),
):
    return crud.get_publication_affiliations(pagination, db)
